...
import time
import os
from typing import Tuple, Union, List, Optional, Dict, Any, Sequence
...
def run_command(
    ...
    try:
        # 1. Komut formatını belirle ve type-safe şekilde işle
        ...
        safe_log_cmd = sanitize_log_message(
...
def run_commands_parallel(
    commands: Sequence[Union[str, List[str]]], timeout: int = DEFAULT_TIMEOUT, **kwargs: Any
    ...
    results: List[Optional[Tuple[str, str, int]]] = [None] * len(commands)
    ...
    # Type assertion: we know all results are filled
    return [r for r in results if r is not None]


def safe_command_output(
    command: Union[str, List[str]], default: str = "N/A", strip: bool = True, **kwargs: Any
) -> str:
    """
...
def run_command_json(command: Union[str, List[str]], **kwargs: Any) -> Optional[Any]:
    """
...
def clear_command_cache() -> None:
    ...
    Examples:
        >>> clear_command_cache()
        >>> # Cache temizlendi, komut kontrolleri yeniden yapılacak
    """
...
if __name__ == "__main__":
    ...
    # Test 4: Paralel komutlar
    print("4. Paralel komutlar:")
    parallel_commands = [["echo", "test1"], ["echo", "test2"], ["echo", "test3"]]
    parallel_results = run_commands_parallel(parallel_commands)
    for i, (out, err, code) in enumerate(parallel_results, 1):
        print(f"   Komut {i}: {out.strip()} (kod: {code})")
    print()
